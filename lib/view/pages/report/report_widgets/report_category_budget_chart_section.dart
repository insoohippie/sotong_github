...
class _ReportCategoryBudgetChartSectionState
  ...
  Widget build(BuildContext context) {
    ...
    if (currentData.isNotEmpty) {
      final totalBudget = currentData
          .map((e) => e['budget'] as int)
          .fold<int>(0, (s, v) => s + v);
      final totalSpent = currentData
          .map((e) => e['spent'] as int)
          .fold<int>(0, (s, v) => s + v);
      chartData.add({
        'category': '총예산',
        'budget': totalBudget,
                ...
                builder: (context, constraints) {
                  ...
                  if (_showPopup && _selectedChartCategory != null) {
                    final match = displayData.firstWhere(
                          (item) =>
                      item['category'] == _selectedChartCategory,
                      ...
                      Positioned.fill(
                        child: BarChart(
                          BarChartData(
                            ...
                            barGroups:
                            _buildBarGroups(displayData, maxY),
                          ),
  ...
  List<BarChartGroupData> _buildBarGroups(
      List<Map<String, dynamic>> data, double maxY) {
    return List.generate(data.length, (index) {
      ...
      final lowerHeight =
      math.min(effectiveSpent, effectiveBudget);
      final upperHeight =
      math.max(effectiveSpent, effectiveBudget);
      ...
      final upperSegHeight =
      (upperHeight - lowerHeight).clamp(0.0, maxY);
      if (upperSegHeight > epsilon) {
        final upperColor =
        isOverBudget ? hoverRed : Colors.grey[300]!;
        stackItems.add(
