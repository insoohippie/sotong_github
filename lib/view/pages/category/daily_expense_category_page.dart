...
/// 일 변동소비 카테고리 관리 페이지
class DailyExpenseCategoryPage extends StatefulWidget {
  const DailyExpenseCategoryPage({Key? key}) : super(key: key);

  @override
  State<DailyExpenseCategoryPage> createState() =>
      _DailyExpenseCategoryPageState();
}

class _DailyExpenseCategoryPageState extends State<DailyExpenseCategoryPage> {
  final List<CatPreset> _dailyExpenseCategories = [
    const CatPreset('식비', Icons.restaurant_rounded),
    const CatPreset('카페', Icons.local_cafe_rounded),
    const CatPreset('쇼핑', Icons.shopping_bag_rounded),
    const CatPreset('여가', Icons.sports_esports_rounded),
  ];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.separated(
        ...
        itemCount: _dailyExpenseCategories.length,
        separatorBuilder: (_, __) => const SizedBox(height: 12),
        itemBuilder: (context, i) {
          final item = _dailyExpenseCategories[i];
          return Dismissible(
            key: Key(item.name),
            ...
            onDismissed: (direction) {
              setState(() {
                _dailyExpenseCategories.removeAt(i);
                _categoryEnabled.removeAt(i);
                CategoryStateManager.updateDailyExpenseStates(_categoryEnabled);
              });
                    ...
                    Switch(
                      ...
                      onChanged: (value) {
                        setState(() {
                          ...
                          CategoryStateManager.setDailyExpenseState(i, value);
                        });
                      },
      ...
      MaterialPageRoute(
        builder: (context) => _AddEditCategoryPage(
          onSave: (name, icon, color) {
            setState(() {
              _dailyExpenseCategories.add(CatPreset(name, icon));
  ...
  void _editCategory(int index) {
    final item = _dailyExpenseCategories[index];
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => _AddEditCategoryPage(
          ...
          onSave: (name, icon, color) {
            setState(() {
              _dailyExpenseCategories[index] = CatPreset(name, icon);
            });
          },
        ),
      ),
...
class _AddEditCategoryPageState extends State<_AddEditCategoryPage> {
  ...
  void initState() {
    ...
    if (widget.initialName != null) {
      ...
      _selectedIcon = widget.initialIcon ?? Icons.restaurant_rounded;
