...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    // 하경
    // todaySpending, todayIncome → 금액 표시용
    // hasSpending, hasIncome → 엔트리 존재 여부 기준으로 수정
    // isUnrecorded → 자동 생성된 미기록일자 여부
    // isOverLimit → 실제 기록일 때만 초과 판단

    final todayIncome = vm.todayIncome;
    final todaySpending = vm.todaySpending;
    final day = vm.selectedDayRecord;

    final hasIncome = day?.incomeEntries.isNotEmpty ?? false;
    final hasSpending = day?.spendingEntries.isNotEmpty ?? false;

    final todaySpendingText = vm.todaySpendingText;
    final isUnrecorded = (day != null && day.isAutoGenerated && hasSpending);

    final isNoSpendingDay = hasSpending &&
        !(day?.isAutoGenerated ?? false) &&
        (day?.spendingEntries.every((e) =>
        e.categoryKey == 'no_spending' || e.category == '무지출') ?? false);
