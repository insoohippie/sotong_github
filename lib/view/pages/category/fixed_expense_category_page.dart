...
/// 월 고정소비 카테고리 관리 페이지
class FixedExpenseCategoryPage extends StatefulWidget {
  const FixedExpenseCategoryPage({Key? key}) : super(key: key);

  @override
  State<FixedExpenseCategoryPage> createState() =>
      _FixedExpenseCategoryPageState();
}

class _FixedExpenseCategoryPageState extends State<FixedExpenseCategoryPage> {
  final List<CatPreset> _fixedExpenseCategories = [
    const CatPreset('주거', Icons.home_rounded),
    const CatPreset('통신', Icons.wifi_rounded),
    const CatPreset('교통', Icons.directions_bus_rounded),
    const CatPreset('구독', Icons.subscriptions_rounded),
  ];
  ...
  void initState() {
    super.initState();
    _categoryEnabled = List.from(
      CategoryStateManager.fixedExpenseEnabledStates,
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.separated(
        ...
        itemCount: _fixedExpenseCategories.length,
        separatorBuilder: (_, __) => const SizedBox(height: 12),
        itemBuilder: (context, i) {
          final item = _fixedExpenseCategories[i];
          return Dismissible(
            key: Key(item.name),
            ...
            onDismissed: (direction) {
              setState(() {
                _fixedExpenseCategories.removeAt(i);
                _categoryEnabled.removeAt(i);
                CategoryStateManager.updateFixedExpenseStates(_categoryEnabled);
              });
                    ...
                    Switch(
                      ...
                      onChanged: (value) {
                        setState(() {
                          ...
                          CategoryStateManager.setFixedExpenseState(i, value);
                        });
                      },
      ...
      MaterialPageRoute(
        builder: (context) => _AddEditCategoryPage(
          onSave: (name, icon, color) {
            setState(() {
              _fixedExpenseCategories.add(CatPreset(name, icon));
  ...
  void _editCategory(int index) {
    final item = _fixedExpenseCategories[index];
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => _AddEditCategoryPage(
          ...
          onSave: (name, icon, color) {
            setState(() {
              _fixedExpenseCategories[index] = CatPreset(name, icon);
            });
          },
        ),
      ),
...
class _AddEditCategoryPageState extends State<_AddEditCategoryPage> {
  ...
  void initState() {
    ...
    if (widget.initialName != null) {
      ...
      _selectedIcon = widget.initialIcon ?? Icons.home_rounded;
