...
class CommunicationViewModel extends ChangeNotifier {
  ...
  List<Map<String, dynamic>> emotionTop3Stats(String period) {
    ...
    for (final day in days) {
      ...
      countByEmotion[emotion] = (countByEmotion[emotion] ?? 0) + 1;
      totalByEmotion[emotion] =
          (totalByEmotion[emotion] ?? 0) + day.totalSpendingAmount;
    }

    final list = countByEmotion.entries.map((entry) {
      final emotion = entry.key;
      final count = entry.value;
      final total = totalByEmotion[emotion] ?? 0;
