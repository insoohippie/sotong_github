...
/// Monthly recurring consume definition applied to one or more months.

@immutable
class MonthlyConsume {
  const MonthlyConsume({
    required this.id,
  ...
  factory MonthlyConsume.newForMonths({
    required String id,
  ...
  MonthlyConsume deactivate() {
    return copyWith(isActive: false);
  }

  MonthlyConsume removeMonths(Iterable<DateTime> months) {
  ...
  MonthlyConsume addMonths(Iterable<DateTime> months) {
  ...
  factory MonthlyConsume.fromMap(String id, Map<String, dynamic> map) {
    return MonthlyConsume(
      id: id,
