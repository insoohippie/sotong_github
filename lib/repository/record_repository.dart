...
class RecordRepository {
  ...
  String _autoFillKey(String uid) => '$uid:autoFill:lastDate'; // 세은님 추가 부분
  ...
  // 세은님 추가 부분
  DateTime? getLastAutoFillDate() {
    final uid = _uid;
    if (uid == null) return null;
    final raw = _cacheBox.get(_autoFillKey(uid));
    if (raw is String) {
  ...
  // 세은님 추가 부분
  Future<void> setLastAutoFillDate(DateTime date) async {
    final uid = _uid;
    if (uid == null) return;
    await _cacheBox.put(_autoFillKey(uid), date.toIso8601String());
  }
  ...
  Future<void> hydrateAllFromRemote() async {
    ...
    try {
      ...
      debugPrint(
        '[RecordRepository] hydrate complete: ${snaps.docs.length} months cached',
      );
    } catch (e) {
  ...
  Future<void> saveMonthlyRecord(
    ...
    if (!localMode && isOnline) {
      ...
      if (ok) {
        _setDirty(monthKey, false);
        await _syncDirtyMonths(excludeMonthKey: monthKey);
      } else {
        _setDirty(monthKey, true);
      }
    }
  ...
  Future<void> upsertSpendingForDate({
    ...
    final updatedDay = existingDay.copyWith(
      ...
      isAutoGenerated: false, // 세은님 추가 부분
    );
  ...
  // 세은님 추가 부분
  Future<bool> ensureAutoSpendingDay({
    required DateTime date,
    required List<RecordEntry> spendingEntries,
  }) async {
    final uid = _uid;
    ...
    final dateKey = DateFormat('yyyy-MM-dd').format(date);
    final monthly = await loadMonthlyRecord(monthKey);
    final existing = monthly.days[dateKey];
    if (existing != null && !existing.isAutoGenerated) {
      return false;
    }
    if (existing != null && existing.isAutoGenerated) {
      return false;
    }
    final amount = spendingEntries.fold<int>(
      0,
      (total, entry) => total + entry.amount.round(),
    );
    final day = DayRecord(
      date: date,
      ...
      emotion: '',
      comment: '',
      spendingEntries: List<RecordEntry>.from(spendingEntries),
      incomeEntries: const [],
      isAutoGenerated: true,
    );
    final updatedDays = Map<String, DayRecord>.from(monthly.days)
      ..[dateKey] = day;
    final updatedMonthly = monthly.copyWith(days: updatedDays);
    await saveMonthlyRecord(updatedMonthly);
    return true;
  }
  ...
  Future<void> syncDirtyMonths() => _syncDirtyMonths();

  Future<void> _syncDirtyMonths({String? excludeMonthKey}) async {
    final uid = _uid;
    if (uid == null) {
    ...
    if (!isOnline) {
      debugPrint('[RecordRepository] syncDirtyMonths aborted: offline');
      return;
    }
    final prefix = '$uid:';
    ...
    for (final dirtyKey in dirtyKeys) {
      final monthKeyWithPrefix = dirtyKey.substring(
        0,
        dirtyKey.length - dirtySuffix.length,
      );
      final monthKey = monthKeyWithPrefix.substring(prefix.length);
      if (monthKey == excludeMonthKey) continue;
      if (!_isDirty(monthKey)) continue;
